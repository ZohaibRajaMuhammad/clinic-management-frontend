...
import { resetPasswordApi, setPasswordApi } from "@/services/authapi";
...
export default function Resetpassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!setToken) {
      console.error("set token missing in URL");
      setErrors({ token: "Invalid or missing reset token." });
      return;
    }

    try {
      const response = await setPasswordApi(form.password, setToken);
     
      
      if (response?.message === "Password set successfully, account activated!") {
        router.push("/login"); 
      } else {
        console.error("Unexpected response:", response);
      }
    } catch (error) {
      console.error("set password API error:", error.message);
    }
  };
